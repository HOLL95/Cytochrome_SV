...
def empty(arg):
    return arg
plot_locs={"E_0":{"E0_mean":{"row":"row1", "col":0, "col_end":False, "func":cyt.e_nondim},
                "E0_std":{"row":"row2", "col":0, "col_end":False, "func":cyt.e_nondim},
                "E0_skew":{"row":"row3", "col":0, "col_end":True, "func":cyt.e_nondim}},
        "k_0":{"k0_shape":{"row":"row2", "col":1, "col_end":True, "func":cyt.t_nondim},
                "k0_scale":{"row":"row1", "col":1, "col_end":False, "func":cyt.t_nondim}}}
...
for parameter in original_params:
    ...
    for disp_param in disped_params:
        ...
        for i in range(0, len(params[parameter][disp_param])):
            ...
            syn_harmonics=harms.generate_harmonics(time_results, cyt.i_nondim(syn_time*1e6))
            #plt.subplot(2,2,j)
            #plt.plot(syn_time, alpha=0.5)
            func=plot_locs[parameter][disp_param]["func"]
            values=func(cyt.values)
            weights=cyt.weights
            disp_ax=figure.axes_dict[plot_locs[parameter][disp_param]["row"]][(num_harms*plot_locs[parameter][disp_param]["col"])]
            if parameter=="k_0":
...
save_path="ramped_harmonic_parameter_scans_irreversible.png"
plt.show()

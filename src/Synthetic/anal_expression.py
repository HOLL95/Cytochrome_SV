...
freq_range=[(10**x) for x in np.arange(0,6)]
...
,   #(ac voltage amplitude - V) freq_range[j],#
    ...
    alphas=np.linspace(0, 1, 400)
    ratio=np.zeros(len(alphas))
    ratio_2=np.zeros(len(alphas))
    hann=np.hanning(len(dim_i))
    fft=np.fft.fft(np.multiply(dim_i, hann))
    freqs=np.fft.fftfreq(len(dim_i), time_range[2]-time_range[1])
    desired_harms=list(range(2, 5))
    peak_heights=np.zeros(len(desired_harms))
    harms=harmonics(desired_harms, param_list["omega"], 0.1)
    anal_harms=harms.generate_harmonics(time_range, dim_i)
    numerical_harms=harms.generate_harmonics(numerical_time, synthetic_data)
    #plt.plot(e_val,anal_harms[0,:])
    #plt.plot(numerical.define_voltages(), numerical_harms[0,:])
    #plt.show()
    for i in range(0, len(desired_harms)):
        filter_val=freq_range[lcv_1]/5

        #plt.plot((fft)
        peak_heights[i]=max(abs(fft[np.where((freqs>((freq_range[lcv_1]*desired_harms[i])-filter_val)) & (freqs<((freq_range[lcv_1]*desired_harms[i])+filter_val)))]))


    for i in range(0, len(alphas)):
        anal.nd_param.alpha=alphas[i]
        ratio[i]=abs(anal.p_m(2))/abs(anal.p_m(3))
        ratio_2[i]=abs(anal.p_m(4))/abs(anal.p_m(3))
    #print(peak_heights[2]/peak_heights[1])
    plt.subplot(2, 1, 1)
    plt.axhline(peak_heights[0]/peak_heights[1])
    #plt.subplot(2, num_freqs/2, lcv_1+1)
    plt.xlabel("$\\alpha$")
    plt.plot(alphas, ratio)
    plt.subplot(2, 1, 2)
    plt.axhline(peak_heights[2]/peak_heights[1])
    plt.xlabel("$\\alpha$")
    #plt.subplot(2, num_freqs/2, lcv_1+1)
    plt.plot(alphas, ratio_2)
    plt.show()

...
for regime in ["irreversible", "reversible"]:
    ...
    for row in ["row1", "row2"]:
        extra_plot_dict[row]= inset_axes(figure.axes_dict[row][3], width="35%", height="35%", loc=3, borderpad=1.3)
        for label in (extra_plot_dict[row].get_xticklabels() + extra_plot_dict[row].get_yticklabels()):
            label.set_fontsize(6)
        extra_plot_dict[row].tick_params(direction='in', length=1, width=1)


    if regime=="reversible":
        scale_scan_vals=np.flip([50, 100, 500])
        DCV_scan_rate=np.power(10,np.linspace(-1, 2, 20))
        plot_scan_rate=DCV_scan_rate[len(DCV_scan_rate)//2]
        k0_val=100
    elif regime=="irreversible":
        scale_scan_vals=np.flip([0.5, 1, 2])
        DCV_scan_rate=np.power(10,np.linspace(-2, 1, 20))
        plot_scan_rate=DCV_scan_rate[len(DCV_scan_rate)//2]
        k0_val=1
            ...
            for i in range(0, len(params[parameter][disp_param])):
                ...
                if parameter=="k_0":
                    ...
                    for q in range(0, len(DCV_scan_rate)):
                        ...
                        syn_time=DCV_new.i_nondim(DCV_new.test_vals(sim_params, "timeseries"))*1e6
                        print(DCV_scan_rate[q], "H(O)")
                        if DCV_scan_rate[q]==plot_scan_rate:
                            syn_time_for_mini_plot=syn_time
                            syn_volts_for_mini_plot=cyt.e_nondim(cyt.define_voltages())
                    ...
                    syn_time=cyt.i_nondim(cyt.test_vals(sim_params, "timeseries"))*1e6
                ...
                if parameter=="k_0":
                    extra_plot_dict[plot_locs[parameter][disp_param]["row"]].plot(syn_volts_for_mini_plot, syn_time_for_mini_plot[:-1])
                    extra_plot_dict[plot_locs[parameter][disp_param]["row"]].yaxis.set_major_formatter(FormatStrFormatter('%.0f'))
                    if i==0:
                        current_ax.scatter(np.log10(DCV_scan_rate),  DCV_new.e_nondim(peak_pos[0,:]), edgecolors=get_colours[i],marker="D", s=5, label="Oxidation V", facecolors='none')
                        ...
                        current_ax.scatter(np.log10(DCV_scan_rate), DCV_new.e_nondim(peak_pos[0,:]), edgecolors=get_colours[i],marker="D", s=5, facecolors='none')

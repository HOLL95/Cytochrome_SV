...
results_mat=np.zeros((20, 20))
for i in range(0, len(k0_vals)):
    for j in range(0, len(shape_linspace)):
        val_dict["k0_scale"]=k0_vals[i]
        val_dict["k0_shape"]=shape_linspace[i]
        val_dict["Ru"]=100

        freq_min=k0_exps[i]-2
        freq_max=k0_exps[i]+2
        freqs=np.linspace(freq_min, freq_max, 20)
        ...
        results_mat[i,j]=freqs[(np.where(errors==max(errors)))]
        #plt.show()
...
np.save("Scan_results.txt", results_mat)

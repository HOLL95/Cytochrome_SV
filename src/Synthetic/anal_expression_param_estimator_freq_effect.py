...
num_oscillations=[10, 50, 100, 200, 300, 400]
...
estimate_array=np.zeros(len(freq_range))
big_array=[]
for num_peaks in num_oscillations:
    for lcv_1 in range(0, num_freqs):
        ...
        for lcv_2 in range(0, len(d_E_vals)):
            ...
            noisy_i=numerical.add_noise(i_val, 0.000*max(i_val))
        ...
        #print(e0_estimates)
        estimate_array[lcv_1]=np.mean(e0_estimates[~np.isnan(e0_estimates)])
        #print(list(estimate_array))
        big_array.append(estimate_array)
    print(num_peaks)
    plt.plot(freq_range, estimate_array, label=num_peaks)
np.save("Parameter_inference_changing_freqs", big_array)
plt.legend()
plt.show()

...
for lcv_1 in range(0, num_freqs):
    param_list={
        'E_start':  -0.25,#-0.0770389330794117, #(starting dc voltage - V)
        'E_reverse':  0.25,#0.0770389330794117,    #  (reverse dc voltage - V)
        'omega':freq_range[lcv_1],#8.88480830076,  #    (frequency Hz)
        'd_E': 0.25,#0.0770389330794117
   #(ac voltage amplitude - V) freq_range[j],#
    ...
    #plt.plot(numerical.e_nondim(numerical.define_voltages()), synthetic_data)

...
for i in range(1, 2):
    ...
    single_oscillation_plot(time_results, fourier_arg, colours[0], label="Experiment Data")
    ...
    for i in range(0, len(SV_params)):
        ...
        single_oscillation_plot(time_results, cyt.top_hat_filter(tests[i]), colours[i+1], label="Parameter_set_"+str(i+1), alpha=1-((i+1)*0.2))
    plt.xlabel("Nondimensional oscillation period")
    plt.ylabel("Nondimensional current")
    ...
    #plt.plot(cmaes_test)
    #plt.show()
    h_class.plot_harmonics(times=time_results, Experiment_Data_time_series=current_results,  xaxis=voltage_results,ylabel="Nondimensional current",xlabel="Nondimensional voltage", alpha_increment=0.1, **{"Parameter_set_"+str(i+1)+"_time_series":tests[i] for i in range(0, len(tests))})

...
for i in range(1, 4):
    ...
    plt.plot(voltage_results, true_data, label="Experiment", alpha=0.7)
    ...
    cmaes_time=cyt.test_vals(cmaes_results, likelihood="timeseries", test=False)
    plt.subplot(2, 3, i+3)
    plt.plot(voltage_results, cyt.farad_current, label="Predicted Faradaic")
    plt.plot(voltage_results, cyt.pred_cap, label="Predicted capacitance")
    plt.plot(voltage_results, true_data, label="Experiment", alpha=0.7)
    plt.xlabel("Nondim voltage")
    plt.ylabel("Nondim current")
    plt.legend()

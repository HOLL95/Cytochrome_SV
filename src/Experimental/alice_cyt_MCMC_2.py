...
for i in range(2, 3):
    ...
    param_bounds={
        ...
        'CdlE2': [-5e-3,5e-3],#0.000245772700637,
        'CdlE3': [-5e-4,4e-4],#1.10053945995e-06,
    ...
    cmaes_params=[-0.07688974263309035, 0.027377978478251054, 124.68226016812127, 82.65176202767852, 0.0007178487287654843, -0.005485275534002848, 0.0012629554419994436, -1.721924369159128e-05, 1.999999965112904e-11, 9.014907846043297, 5.269613328541364, 4.96139021671786, 0.599999897192592]
    ...
    for q in range(0, num_runs):
        ...
        filename="alice_cyt_{0}_MCMC".format(i)
        save_file=filename
        if rhat_mean<1.08:
            f=open(save_file, "wb")
            np.save(f, chains)

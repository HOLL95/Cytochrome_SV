...
for i in range(0, len(frequencies),3):
    ...
    for j in range(1, 2):
        ...
        """
            h_class.plot_harmonics(times=time_results, xlabel="Nondim time", ylabel="Nondim current", **{str(frequencies[i])+"Hz_time_series":current_results, "legend":{"ncol":3, "loc":"upper center"}})
            fourier_idx=np.where((h_class.f>0) & (h_class.f<h_class.harmonics[-1]+1))
            plt.plot(h_class.f[fourier_idx], np.cbrt(np.real(h_class.Y[fourier_idx])), label=str(frequencies[i])+"Hz")
            plt.scatter(h_class.harmonics, np.cbrt(amps), color=cycle[c_counter])
            """
        ...
        while fit_success==False:
            try:
                ...
                popt, pcov=curve_fit(damped_osc, h_class.harmonics,amps)
                fit_success=True
            except:
                start_harm+=1
        ...
        fourier_idx=np.where((h_class.f>1.5) & (h_class.f<h_class.harmonics[-1]+1))
